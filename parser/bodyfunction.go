...
/* parseBodyFunctionAssemblyBlock parses an assembly block. It assumes that the
 * assembly block header was parsed, and the actual assembly code is known. This
 * function takes care of what goes under that function (inputs, outputs,
 * squashed registers).
 */
func (parser *Parser) parseBodyFunctionAssemblyBlock (
        parentIndent int,
        parent      *Block,
        code         string,
) (
...
/* parseBodyFunctionAssemblyArgumentFor parses the assembly block argument for
 * the specified assembly block.
 */
func (parser *Parser) parseBodyFunctionAssemblyArgumentFor (
        parentIndent   int,
        parent        *Block,
        assemblyBlock *AssemblyBlock,
) (
        ...
        argumentType := parser.token.StringValue
        parser.nextToken()
        
        if argumentType == ">" || argumentType == "<" {
                var assemblyArgument AssemblyArgument
        
                // get the register this argument applies to
                if !parser.expect(lexer.TokenKindString) {
                        return false, nil
                }

                assemblyArgument.register = parser.token.StringValue
                ...
                worked, err = parser.parseBodyFunctionStatementArgument (
                        parentIndent, parent)
                if err != nil || !worked { return false, err }

                // we don't expect anything afterwards
                parser.nextToken()
                if !parser.expect(lexer.TokenKindNone) {
                        return false, nil
                }

                // add the argument to the assembly block
                if (argumentType == ">") {
                        assemblyBlock.inputs = append (
                                assemblyBlock.inputs,
                                assemblyArgument)
                } else {
                        assemblyBlock.outputs = append (
                                assemblyBlock.outputs,
                                assemblyArgument)
                }
        
        } else if argumentType == "%" {
                // get all clobbered registers
                for {
                        if !parser.expect (
                                lexer.TokenKindString,
                                lexer.TokenKindNone,
                        ...
                        if parser.token.Kind == lexer.TokenKindNone { break }

                        // add to assembly block
                        assemblyBlock.clobbered = append (
                                assemblyBlock.clobbered,
                                parser.token.StringValue)
                }
                ...
                parser.printError (
                        parser.token.Column,
                        "unknown assembly argument type symbol '" +
                        parser.token.StringValue + "',",
                        "use either '>', '<', or '%'")
        }
